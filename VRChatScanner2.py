...
user_agent = 'VRC Scanner Tool/OSINT Squad (Version: 1.0.0; Contact: errornoname666@gmail.com)'
...
def fancy_welcome(version, developers=None):
    if developers is None:
        developers = [
            {'name': 'ErrorNoName', 'role': 'Lead Developer'},
    ...
    # ASCII Art text for "Welcome to OSINT Squad"
    welcome_text = r"""
  ______    ______   ______  __    __  ________         ______    ______   __    __   ______   _______  
 /      \  /      \ /      |/  \  /  |/        |       /      \  /      \ /  |  /  | /      \ /       \ 
/$$$$$$  |/$$$$$$  |$$$$$$/ $$  \ $$ |$$$$$$$$/       /$$$$$$  |/$$$$$$  |$$ |  $$ |/$$$$$$  |$$$$$$$  |
$$ |  $$ |$$ \__$$/   $$ |  $$$  \$$ |   $$ |         $$ \__$$/ $$ |  $$ |$$ |  $$ |$$ |__$$ |$$ |  $$ |
$$ |  $$ |$$      \   $$ |  $$$$  $$ |   $$ |         $$      \ $$ |  $$ |$$ |  $$ |$$    $$ |$$ |  $$ |
...
$$    $$/ $$    $$/ / $$   |$$ | $$$ |   $$ |         $$    $$/ $$ $$ $$< $$    $$/ $$ |  $$ |$$    $$/ 
 $$$$$$/   $$$$$$/  $$$$$$/ $$/   $$/    $$/           $$$$$$/   $$$$$$  | $$$$$$/  $$/   $$/ $$$$$$$/  
    ...
    # Thank you message
    thank_you_text = "POTATOS Thank you for using the OSINT VRC Scanner Tool"
...
# VRCHAT API
show_notification('OSINT Squad', 'The LocalAvatarLogger script was launched successfully.')
advertise_OSINT_gang()
#HERE PUT AUTH CODE
...
def download_entity_image(entity_id, entity_type):
    ...
    for entity_info in entities_info:
        if entity_info.get('id') == entity_id:
            ...
            try:
                headers = {'User-Agent': 'VRC Scanner Tool / OSINT Squad'}
...
# Main Menu Principal
def main_menu():
    auth_cookie = get_auth_cookie()  # Assurez-vous de récupérer correctement le cookie d'authentification ici

    while True:
        print(f"{Fore.RED}\nNasa get Hacked by ErrorNoName {Fore.GREEN}for question dm Discord : kaichisama.{Style.RESET_ALL}")
        print(f"{Fore.LIGHTMAGENTA_EX}Join : https://discord.gg/nEcDCWVwWW{Style.RESET_ALL}")
        print(f"{Fore.LIGHTMAGENTA_EX}Powered by OSINT Squad Devs : ErrorNoName / >_Unknown User{Style.RESET_ALL}")
        print(f"\n{Fore.GREEN}♥ ErrorNoName Menu UwU ♥{Style.RESET_ALL}:")
        ...
        print("6. Backup Account")
        print("7. Search VRChat Users")
        print("8. Exit")  # Mettez à jour le numéro des options ici
        ...
        if choice == "1":
            ...
            print("you get Rickrolled by OSINTTools Dev Team <3")
            rickroll()
            ...
            search_vrchat_users(auth_cookie)
        elif choice == "8":
            break
        else:
            print("Invalid option, please try again.")
...
def backup_account():
    ...
    if os.path.exists(auth_cookie_path):
        with open(auth_cookie_path, 'r') as file:
            ...
            auth_cookie = next((part.split('=')[1] for part in cookie_content.split('; ') if part.startswith('auth=')),
                               None)
            if not auth_cookie:
                print("Auth cookie value not found in the file.")
                return None
    ...
    # Save friends list
    friends_list = get_vrchat_friends()
    with open(os.path.join(backup_folder, 'friends_list.json'), 'w') as file:
        json.dump(friends_list, file, indent=4)

    # Save worlds list
    worlds_list = get_vrchat_worlds(auth_cookie)
    with open(os.path.join(backup_folder, 'worlds_list.json'), 'w') as file:
        json.dump(worlds_list, file, indent=4)

    # Save avatars list
    avatars_list = get_vrchat_avatars(auth_cookie)
    with open(os.path.join(backup_folder, 'avatars_list.json'), 'w') as file:
        json.dump(avatars_list, file, indent=4)
...
def get_vrchat_worlds(auth_cookie):
    ...
    if response.status_code == 200:
        worlds_list = response.json()
        with open('backup/worlds_list.json', 'w') as file:
            json.dump(worlds_list, file, indent=4)
        print("Worlds list saved.")
        return worlds_list
    else:
        print(f"Error retrieving worlds list: {response.status_code}")
        return None

def get_vrchat_avatars(auth_cookie):
    url = "https://api.vrchat.cloud/api/1/avatars"
    headers = {"User-Agent": user_agent}
    cookies = {"auth": auth_cookie}
    ...
    if response.status_code == 200:
        avatars_list = response.json()
        with open('backup/avatars_list.json', 'w') as file:
            json.dump(avatars_list, file, indent=4)
        print("Avatars list saved.")
        return avatars_list
    else:
        print(f"Error retrieving avatars list: {response.status_code}")
        return None

def get_auth_cookie():
    auth_cookie_path = 'Logs/AuthCookie.bin'  # Assurez-vous que le chemin est correct
...
def search_vrchat_users(auth_cookie):
    # Demander le nom d'utilisateur à rechercher
    search_query = input("Enter the username to search for: ")
    ...
    save_path = os.path.join(save_folder, "users_list.json")

    url = f"https://api.vrchat.cloud/api/1/users?search={search_query}"
    headers = {"User-Agent": user_agent}
    cookies = {"auth": auth_cookie}
    ...
    if response.status_code == 200:
        new_users_list = response.json()

        # Charger les données existantes du fichier s'il existe
        existing_users_list = []
        if os.path.exists(save_path):
            with open(save_path, 'r') as existing_file:
                existing_users_list = json.load(existing_file)

        # Ajouter les nouveaux utilisateurs à la liste existante
        existing_users_list.extend(new_users_list)

        # Enregistrer la liste mise à jour dans le fichier
        with open(save_path, 'w') as file:
            json.dump(existing_users_list, file, indent=4)

        if new_users_list:
            print(f"Users list saved to {save_path}.")
            return existing_users_list
        else:
            print(f"No users found for the search query: {search_query}")
            return None
    else:
        print(f"Error retrieving users list: {response.status_code}")
        return None
